...
@section('buttons')
  <a href="{{route('admin.brands.create')}}" class="btn btn-primary"><i class="fa fa-plus"></i> Добавить</a>
@endsection
      ...
      <table id="data" class="table table-hover table-bordered text-center">
        ...
        <tbody>
          @forelse ($brands as $brand)
            <tr>
              <td>{{$brand->name}}</td>
              <td><img class="img-sm" src="{{asset("images/$brand->image")}}" alt="{{$brand->name}}" /></td>
              <td>
                <a href="{{route('admin.brands.edit', ['brand' => $brand->id])}}" class="btn btn-info"><i class="fa fa-edit"></i></a>
                <a href="{{route('brands.single', ['slug' => Str::slug($brand->id.' '.$brand->slug)])}}" target="_blank" class="btn btn-success"><i class="fa fa-eye"></i></a>
              </td>
              <td>
                <button class="btn btn-danger del" data-toggle='modal' data-id="{{$brand->id}}" data-target="#trashModal"><i class="fa fa-trash"></i></button>
              </td>
            </tr>
          @empty
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                <form method="POST" id="forceDelete" action="{{route('admin.brands.destroy', ["brand" => $brand->id + 1])}}">   
                  @csrf
                  @method('DELETE')
